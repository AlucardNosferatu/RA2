	...
	TechnoClass* const pThis, WeaponTypeClass const* const pWeapon)
{
	...
	while(count-- > 0) {
		pThis->DecreaseAmmo();
		//This is the native DA function inside gamemd.exe which only support Ammo-=1
	}
}
...
void TechnoExt::ExtData::RecalculateStats() {
	...
	if (auto const pFoot = abstract_cast<FootClass*>(pThis)) {
		Debug::Log("[AttachEffect]Before applying SM AE :%f...\n", pFoot->SpeedMultiplier);
		Debug::Log("[AttachEffect]pFoot ID:%s...\n", pFoot->get_ID());
		Debug::Log("[AttachEffect]pFoot Current Speed:%d...\n", pFoot->GetCurrentSpeed());
		Debug::Log("[AttachEffect]pFoot Speed Percentage:%f...\n", pFoot->SpeedPercentage);
		Debug::Log("\n\n");
	}
	else {
		Debug::Log("[AttachEffect]pThis cannot be cast to pFoot\nSM application will be skipped.\n");
	}
	...
	if(auto const pFoot = abstract_cast<FootClass*>(pThis)) {
		...
		Debug::Log("[AttachEffect]After applying SM AE :%f...\n", pFoot->SpeedMultiplier);
		Debug::Log("[AttachEffect]pFoot ID:%s...\n", pFoot->get_ID());
		Debug::Log("[AttachEffect]pFoot Current Speed:%d...\n", pFoot->GetCurrentSpeed());
		Debug::Log("[AttachEffect]pFoot Speed Percentage:%f...\n", pFoot->SpeedPercentage);
		Debug::Log("\n\n");
